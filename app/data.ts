...
const USE_STAGING = 0
const endpoint = USE_STAGING ? 'mortality-staging' : 'mortality'
...
const calculateBaseline = async (
  ...
  const all_data = data[keys[0]].slice(startIdx, n)
  const bl_data = data[keys[0]].slice(startIdx, endIdx + 1)
  ...
  try {
    ...
    data[keys[1]] = prefillUndefined(
      json.y as NumberArray,
      startIdx
    ) as DataVector
    data[keys[2]] = prefillUndefined(json.lower, startIdx) as DataVector
    data[keys[3]] = prefillUndefined(json.upper, startIdx) as DataVector
  } catch (error) {
    console.log(error)
  }

  calculateExcess(data, keys[0])
}
