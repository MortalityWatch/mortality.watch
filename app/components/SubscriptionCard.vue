...
<template>
  <UCard>
    ...
    <div v-else-if="subscriptionStatus" class="space-y-6">
      <!-- Active Subscription -->
      <div v-if="subscriptionStatus.hasSubscription && subscriptionStatus.subscription">
        <div class="space-y-4">
          ...
          <div v-if="subscriptionStatus.subscription.currentPeriodEnd">
            ...
            <p v-if="subscriptionStatus.subscription.daysRemaining !== null" class="text-xs text-gray-500 dark:text-gray-400 mt-1">
              {{ subscriptionStatus.subscription.daysRemaining }} days remaining
            </p>
          </div>

          <div v-if="subscriptionStatus.subscription.cancelAtPeriodEnd" class="p-3 bg-yellow-50 dark:bg-yellow-900/10 rounded-md border border-yellow-200 dark:border-yellow-800">
            <div class="flex items-start gap-2">
              <UIcon name="i-lucide-info" class="w-4 h-4 text-yellow-600 dark:text-yellow-400 mt-0.5 shrink-0" />
          ...
          <div v-if="subscriptionStatus.subscription.status === 'past_due' || subscriptionStatus.subscription.status === 'unpaid'" class="p-3 bg-red-50 dark:bg-red-900/10 rounded-md border border-red-200 dark:border-red-800">
            <div class="flex items-start gap-2">
              <UIcon name="i-lucide-alert-triangle" class="w-4 h-4 text-red-600 dark:text-red-400 mt-0.5 shrink-0" />
      ...
      <div v-else class="space-y-6">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          ...
          <div class="p-4 border-2 border-primary-500 dark:border-primary-400 rounded-lg relative">
            <UBadge color="primary" variant="solid" size="xs" class="absolute -top-2 right-4">
              Save $20
            </UBadge>
            <div class="mb-3">
              <h3 class="text-lg font-semibold mb-1">
