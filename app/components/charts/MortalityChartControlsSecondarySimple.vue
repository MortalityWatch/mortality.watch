...
const selectedType = computed({
  ...
  set: (v: string | { name: string, value: string }) => emit('typeChanged', typeof v === 'string' ? v : v.value)
})

const selectedChartType = computed({
  get: () => chartTypes.find(t => t.value === props.chartType) || chartTypes[0],
  set: (v: string | { name: string, value: string }) => emit('chartTypeChanged', typeof v === 'string' ? v : v.value)
})

const selectedChartStyle = computed({
  get: () => chartStyles.find(t => t.value === props.chartStyle) || chartStyles[0],
  set: (v: string | { name: string, value: string }) => emit('chartStyleChanged', typeof v === 'string' ? v : v.value)
})
...
const selectedStandardPopulation = computed({
  ...
  set: (v: string | { name: string, value: string }) => emit('standardPopulationChanged', typeof v === 'string' ? v : v.value)
})

const selectedBaselineMethod = computed({
  get: () => baselineMethods.find(t => t.value === props.baselineMethod) || baselineMethods[0],
  set: (v: string | { name: string, value: string }) => emit('baselineMethodChanged', typeof v === 'string' ? v : v.value)
})
...
// Tab management - just use a number
const selectedTab = ref(0)

// Create tab items based on showBaseline
...
<template>
  <div class="w-full">
    ...
    <div class="mt-4">
      ...
      <!-- Baseline Tab -->
      <div v-show="props.showBaseline && selectedTab === 2">
        <div class="flex flex-col gap-4">
          <div>
      ...
      <!-- Style Tab -->
      <div v-show="selectedTab === items.length - 1">
        <div class="flex flex-col gap-4">
          <div>
